                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select name="tipo_unidad" id="tipo_unidad" class="form-control @error('tipo_unidad') is-invalid @enderror" required>
                            <option value="unidad" {{ old('tipo_unidad', $medicamento->tipo_unidad) == 'unidad' ? 'selected' : '' }}>Unidades (pastillas, tabletas, cápsulas)</option>
                            <option value="ml" {{ old('tipo_unidad', $medicamento->tipo_unidad) == 'ml' ? 'selected' : '' }}>Mililitros (ml) - Líquidos</option>
                            <option value="gr" {{ old('tipo_unidad', $medicamento->tipo_unidad) == 'gr' ? 'selected' : '' }}>Gramos (gr) - Pomadas, cremas</option>
                            <option value="ampolla" {{ old('tipo_unidad', $medicamento->tipo_unidad) == 'ampolla' ? 'selected' : '' }}>Ampollas</option>
                            <option value="sobre" {{ old('tipo_unidad', $medicamento->tipo_unidad) == 'sobre' ? 'selected' : '' }}>Sobres</option>
                            <option value="otros" {{ old('tipo_unidad', $medicamento->tipo_unidad) == 'otros' ? 'selected' : '' }}>Otros</option>
                        </select>
                        @error('tipo_unidad')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" name="presentacion" class="form-control @error('presentacion') is-invalid @enderror"
                               value="{{ old('presentacion', $medicamento->presentacion) }}" placeholder="Ej: 1000ml, 500mg, 100 unidades">
                        @error('presentacion')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="number" name="cantidad_stock" id="cantidad_stock"
                               ...
                               value="{{ old('cantidad_stock', $medicamento->cantidad_stock) }}" min="0" step="any" required>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Set initial text based on current value
    const initialTipo = tipoUnidadSelect.value;
    if (textos[initialTipo]) {
        stockInfoText.innerHTML = textos[initialTipo].info;
        stockUnitLabel.textContent = textos[initialTipo].label;
    }
