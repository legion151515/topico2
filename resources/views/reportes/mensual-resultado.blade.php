...
@section('page_title', 'Reporte Mensual - ' . $nombreMes . ' ' . $anio)

@section('content')
<div class="card">
    <div class="card-header">
        <h3>Reporte Mensual - {{ $nombreMes }} {{ $anio }}</h3>
        <div style="display: flex; gap: 10px; flex-wrap: wrap;">
            <a href="{{ route('reportes.mensual') }}" class="btn btn-secondary">
                <i class="fas fa-arrow-left"></i> Volver
            </a>
            <form action="{{ route('reportes.mensual.pdf') }}" method="GET" style="display: inline;">
            ...
            <form action="{{ route('reportes.mensual.excel') }}" method="GET" style="display: inline;">
                <input type="hidden" name="mes" value="{{ $mes }}">
                <input type="hidden" name="anio" value="{{ $anio }}">
        ...
        @else
            <div style="text-align: center; padding: 60px 20px;">
                ...
                <p style="margin-top: 20px; color: #666; font-size: 18px;">
                    No hay atenciones registradas para {{ $nombreMes }} {{ $anio }}
                </p>
            </div>
        @endif
    </div>
